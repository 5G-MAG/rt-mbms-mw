...
#else
    if((fd = open(tmp_filename, O_WRONLY | O_CREAT | O_TRUNC , S_IRWXU | S_IRGRP | S_IROTH)) < 0) {
#endif
...
int fdtbasedrecv(int rx_memory_mode, BOOL openfile, flute_receiver_t *receiver) {
  ...
  while(1) {
    ...
    while(next_file != NULL) {
      ...
      if(file->force_expire || ((file->expires < curr_time) && (!receiver->accept_expired_fdt_inst))) {
	...
	if(file->filesystem_location != NULL) {
	  free(file->filesystem_location);
	}							
	...
	*(tmp_filename + (strlen(tmp_filename) - GZ_SUFFIX_LEN)) = '\0';
    ...
    if(!(file->filesystem_location = (char*)calloc((strlen(fullpath) + 1), sizeof(char)))) {
      printf("Could not alloc memory for tmp (filepath)!\n");
    }
    else {
      memcpy(file->filesystem_location, fullpath, strlen(fullpath));
    }
    file->received_at = (unsigned long)time(NULL);
						...
						if(!((retcode == 0) || (retcode == EOF))) {
							...
							while(file != NULL) {
								...
								if(file->toi == rec_toi) {
									...
									if(file->force_expire || file->expires < curr_time) {
													
										if(file->force_expire || !a->alc_a.accept_expired_fdt_inst) {
